...
class CreateHaiGeChargeOrderMappingsTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('haige_charge_order_mappings', function (Blueprint $table) {
            $table->uuid('id')->comment("映射id");
            $table->uuid('evse_id')->comment("充电桩id");
            $table->uuid('port_id')->comment("充电抢id");
            $table->string('code',32)->comment("充电桩code");
    ...
    {
        Schema::dropIfExists('haige_charge_order_mappings');
    }
}
