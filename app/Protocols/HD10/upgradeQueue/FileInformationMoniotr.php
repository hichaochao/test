...
class FileInformationMoniotr implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if($isSuccess[$packageNumber] == 0 || $isSuccess[$packageNumber] == 4){
            if($upgradeInfo->failure_times == 3){
                Log::debug(__NAMESPACE__ .  "/" . __CLASS__."/" . __FUNCTION__ . "@" . __LINE__ . "升级文件信息,失败超过3次, 包序号:$packageNumber ");
                //3次失败,把升级状态更改为结束
                $upgradeInfo->upgrade_state = 2; //超过失败3次,此桩升级结束
                ...
                Log::debug(__NAMESPACE__ .  "/" . __CLASS__."/" . __FUNCTION__ . "@" . __LINE__ . "升级文件信息,失败超过3次, 调用monitor结果res:$res ");
                //判断时候还有没升级的桩,有的话进行升级 EvseController
                $evse = new EvseController;
                $evse->upgradeInfo();

            }else{
                Log::debug(__NAMESPACE__ .  "/" . __CLASS__."/" . __FUNCTION__ . "@" . __LINE__ . "监控,升级文件信息,未收到响应, 包序号:$packageNumber ");
                //失败加一
